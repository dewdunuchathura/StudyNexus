const StudyGoal = require("../models/StudyGoal");
const { sendError, sendSuccess } = require("./responseHelpers");

// =====================
// GET ALL GOALS
// =====================
exports.getAllGoals = async (req, res) => {
  try {
    const query = {};

    // Students can only see their own goals.
    if (req.user.role === "student") {
      query.user = req.user.id;
    }

    const goals = await StudyGoal.find(query)
      .populate("user", "firstName lastName email")
      .sort({ createdAt: -1 })
      .lean();

    return sendSuccess(res, goals);
  } catch (err) {
    console.error(err);
    return sendError(res, "Server Error");
  }
};

// =====================
// CREATE GOAL
// =====================
exports.createGoal = async (req, res) => {
  try {
    const { title, description, status, priority, progress, deadline, assignedTo } = req.body;

    if (!title) {
      return sendError(res, "Title is required", 400);
    }

    // If admin/lecturer sends assignedTo, use it; otherwise use current user.
    const targetUser = req.user.role !== "student" && assignedTo ? assignedTo : req.user.id;

    const newGoal = new StudyGoal({
      title,
      description,
      status,
      priority,
      progress,
      deadline,
      user: targetUser,
    });

    const savedGoal = await newGoal.save();
    const populatedGoal = await StudyGoal.findById(savedGoal._id).populate(
      "user",
      "firstName lastName email"
    );

    return sendSuccess(res, populatedGoal, 201);
  } catch (err) {
    console.error(err);
    return sendError(res, "Server Error");
  }
};

// =====================
// UPDATE GOAL
// =====================
exports.updateGoal = async (req, res) => {
  try {
    const { title, description, status, priority, progress, deadline, assignedTo } = req.body;

    const goal = await StudyGoal.findById(req.params.id);
    if (!goal) return sendError(res, "Goal not found", 404);

    if (goal.user.toString() !== req.user.id && req.user.role === "student") {
      return sendError(res, "Not authorized", 401);
    }

    goal.title = title || goal.title;
    goal.description = description !== undefined ? description : goal.description;
    goal.status = status || goal.status;
    goal.priority = priority || goal.priority;
    goal.progress = progress !== undefined ? progress : goal.progress;
    goal.deadline = deadline !== undefined ? deadline : goal.deadline;

    if (req.user.role !== "student" && assignedTo) {
      goal.user = assignedTo;
    }

    const updatedGoal = await goal.save();
    const populatedGoal = await StudyGoal.findById(updatedGoal._id).populate(
      "user",
      "firstName lastName email"
    );

    return sendSuccess(res, populatedGoal);
  } catch (err) {
    console.error(err);
    return sendError(res, "Server Error");
  }
};

// =====================
// DELETE GOAL
// =====================
exports.deleteGoal = async (req, res) => {
  try {
    const goal = await StudyGoal.findById(req.params.id);
    if (!goal) return sendError(res, "Goal not found", 404);

    if (goal.user.toString() !== req.user.id && req.user.role === "student") {
      return sendError(res, "Not authorized", 401);
    }

    await goal.deleteOne();
    return sendSuccess(res, { message: "Goal deleted" });
  } catch (err) {
    console.error(err);
    return sendError(res, "Server Error");
  }
};

// =====================
// GET LEADERBOARD
// =====================
exports.getLeaderboard = async (req, res) => {
  try {
    const leaderboard = await StudyGoal.aggregate([
      {
        $group: {
          _id: "$user",
          totalGoals: { $sum: 1 },
          completedGoals: { $sum: { $cond: [{ $eq: ["$status", "Completed"] }, 1, 0] } },
          inProgress: { $sum: { $cond: [{ $eq: ["$status", "In Progress"] }, 1, 0] } },
          avgProgress: { $avg: "$progress" },
        },
      },
      {
        $addFields: {
          score: {
            $add: [
              { $multiply: ["$completedGoals", 100] },
              { $round: [{ $ifNull: ["$avgProgress", 0] }, 0] },
            ],
          },
        },
      },
      { $sort: { score: -1, completedGoals: -1 } },
      {
        $lookup: {
          from: "users",
          localField: "_id",
          foreignField: "_id",
          as: "userInfo",
        },
      },
      { $unwind: "$userInfo" },
      {
        $project: {
          _id: 1,
          totalGoals: 1,
          completedGoals: 1,
          inProgress: 1,
          avgProgress: { $round: ["$avgProgress", 1] },
          score: 1,
          firstName: "$userInfo.firstName",
          lastName: "$userInfo.lastName",
          email: "$userInfo.email",
          role: "$userInfo.role",
        },
      },
    ]);

    const ranked = leaderboard.map((entry, i) => ({ ...entry, rank: i + 1 }));
    return sendSuccess(res, ranked);
  } catch (err) {
    console.error(err);
    return sendError(res, "Server Error");
  }
};
